...
CAV.Color.prototype = {
  set: function (a, b) {
    var anima = a.replace("#", ""), c = a.length / 3;
    this.rgba[0] = parseInt(anima.substring(c * 0, c * 1), 16) / 255;
    this.rgba[1] = parseInt(anima.substring(c * 1, c * 2), 16) / 255;
    this.rgba[2] = parseInt(anima.substring(c * 2, c * 3), 16) / 255;
...
CAV.Plane = function (a, b, c, d) {
  ...
  for (a = 0; a < this.segments; a++) for (b = 0; b < this.slices; b++) {
    d = cnima[a + 0][b + 0];
    e = cnima[a + 0][b + 1];
    f = cnima[a + 1][b + 0];
    g = cnima[a + 1][b + 1];
...
export function Victor(container, anitOut,options) {
  if (!!document.createElement("canvas").getContext) {
    ...
    if(options && JSON.stringify(options) !== '{}'){
      console.log(options)
      G.ambient = options.ambient
      G.diffuse = options.diffuse
    }
    function C() {
      F();
      p();
